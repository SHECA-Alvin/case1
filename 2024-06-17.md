...
- **2024-06-14 21:30** 确认问题的原因是新旧CA系统并行运行，流量入口灰度比例分别为旧CA系统50%和新CA系统50%。在主题属性顺序改造过程中，旧CA系统未按BR要求更改，而新CA系统更改有效。在测试验证阶段，测试人员的测试用例仅运行一次且通过，流量被分配到新的CA系统，导致未能发现旧CA系统的改造问题。附件中附有SHECA CA改造过程中的网络拓扑图以供参考。
...
- **2024-06-14 21:30** The reason for the issue has been identified as the parallel operation of the old and new CA systems, with the traffic being split 50% to the old CA system and 50% to the new CA system. During the modification of the order of topic attributes, the old CA system did not make the changes required by BR, while the new CA system implemented the changes effectively. During the testing and validation phase, the testers ran the test cases only once and passed them, resulting in the traffic being allocated to the new CA system. This led to the failure to detect the modification issues in the old CA system. A network topology diagram of the SHECA CA modification process is attached for reference.
