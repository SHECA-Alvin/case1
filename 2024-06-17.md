...
- **2024-06-14 21:30** 问题原因: 由于新旧CA系统并行运行，流量入口灰度比例分别为旧CA系统50%和新CA系统50%。在主题属性顺序改造过程中，旧CA系统未按BR要求更改，而新CA系统更改有效。在测试验证阶段，测试人员的测试用例仅运行一次且通过，流量被分配到新的CA系统，导致未能发现旧CA系统的改造问题。附件中附有SHECA CA改造过程中的网络拓扑图以供参考。
...
- **2024-06-14 21:30** Cause of the issue: Due to the parallel operation of the old and new CA systems, the traffic entry was split 50% to the old CA system and 50% to the new CA system. During the modification of the topic attribute order, the old CA system did not make the changes required by BR, while the new CA system did so effectively. During the testing and validation phase, testers ran the test cases only once and passed them, resulting in traffic being allocated to the new CA system. This caused the modification issues in the old CA system to go unnoticed. A network topology diagram of the SHECA CA modification process is attached for reference.
